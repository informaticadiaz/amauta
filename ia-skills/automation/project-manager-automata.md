...
Escribir en `ia-skills/automation/loop-status.md` antes de disparar:
...
Escribir en `ia-skills/automation/next-prompt.md` sin incrementar `X`:
...
AL TERMINAR (solo si todas las condiciones son verdaderas):
  ...
  1. Actualizar ia-skills/automation/loop-status.md con resultado de la sesión
  2. Escribir ia-skills/automation/next-prompt.md con el prompt para project-manager-automata:
...
1. Escribir en `ia-skills/automation/loop-status.md`:
...
2. Terminar la sesión con el mensaje:
   `"Loop detenido: [razón]. Ver ia-skills/automation/loop-status.md para detalles."`
