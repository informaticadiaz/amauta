...
export async function GET(
  ...
  try {
    ...
    const url = `${getApiUrl()}/api/v1/grupos/${id}/calificaciones${qs ? `?${qs}` : ''}`;
    ...
    console.error('Error en grupos/:id/calificaciones proxy GET:', error);
...
export async function PUT(
  ...
  try {
    ...
    const response = await fetch(
      `${getApiUrl()}/api/v1/grupos/${id}/calificaciones`,
      {
        method: 'PUT',
        headers: {
    ...
    console.error('Error en grupos/:id/calificaciones proxy PUT:', error);
    return NextResponse.json(
      { message: 'Error al guardar calificaciones del grupo' },
      { status: 500 }
    );
  }
}
